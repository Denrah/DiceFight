...
public class AppDemo : MonoBehaviour {
    ...
    void UpdateGallery()
	{
        if (!PointInRect(GuiMousePosition(), rectModeSelect) && !PointInRect(GuiMousePosition(), rectGallerySelectBox ))
        {
            ...
            if (Input.GetMouseButton(Dice.MOUSE_LEFT_BUTTON))
            {
				...
				//Debug.Log("hit = "+galleryDieObject.GetComponent<Die>().hitVector);
	...
	void UpdateRoll()
	{
        ...
        if (Input.GetMouseButtonDown(Dice.MOUSE_LEFT_BUTTON) && !PointInRect(GuiMousePosition(), rectModeSelect))
		{
            ...
            Dice.Roll("2d20", "d20-" + randomColor, spawnPoint.transform.position, Force());
            Dice.Roll("2d12", "d12-" + randomColor, spawnPoint.transform.position, Force());
            Dice.Roll("2d10", "d10-" + randomColor, spawnPoint.transform.position, Force());
            Dice.Roll("2d8", "d8-" + randomColor, spawnPoint.transform.position, Force());
            Dice.Roll("2d6", "d6-" + randomColor, spawnPoint.transform.position, Force());
            Dice.Roll("2d4", "d4-" + randomColor, spawnPoint.transform.position, Force());
		}
        else
	...
	private string CheckSelection(Rect r)
	{
        ...
        if (PointInRect(mp, new Rect(r.xMin + 12, r.yMin + 9, 200, 46)))
        {
            ...
            if (mp.x - r.xMin < 45)
            {
                if (mlb) SetGalleryDie("d4-" + galleryDie.Split('-')[1]);
                status = "d4";
            }
            else
                if (mp.x - r.xMin < 75)
                {
                    ...
                    if (mp.x - r.xMin < 115)
                    {
                        if (mlb) SetGalleryDie("d8-" + galleryDie.Split('-')[1]);
                        status = "d8";
                    }
                    else
                        if (mp.x - r.xMin < 147)
                        {
                            ...
                            if (mp.x - r.xMin < 180)
                            {
                                if (mlb) SetGalleryDie("d12-" + galleryDie.Split('-')[1]);
                                status = "d12";
                            }
                            else
                            {
                                if (mlb) SetGalleryDie("d20-" + galleryDie.Split('-')[1]);
                                status = "d20";
                            }
        }
        else
